...
class CategoryView(View):
    def get(self,request,val):
        ...
        tittle = Products.objects.filter(category=val).values('tittle')
...
class CategoryTittle(View):
    def get(self,request,val):
        product = Products.objects.filter(tittle=val)
        tittle = Products.objects.filter(category=product[0].category).values('tittle')
        return render(request,'app/category.html',locals())
...
def show_cart(request):
    ...
    for p in cart:
        value = p.quantity * p.product.rent_price
        amount = amount + value
